		...
		// end of table view navigation code
			...
			// execute query for table view
				...
				while(($row = db_fetch_array($result)) && ($i < ($FirstRecord + $this->RecordsPerPage))){
					...
					if(($PrintTV || $Print_x) && (isset($_REQUEST['record_selector']) && is_array($_REQUEST['record_selector']) && count($_REQUEST['record_selector'])) && !in_array($row[$FieldCountTV], $_REQUEST['record_selector'])) continue;
